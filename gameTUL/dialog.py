...
class DialogTree:
	def __init__(self, character, player, all_sprites, font, end_dialog):
		...
		self.current_dialog = DialogBoxSprite(self.dialog[self.dialog_index], self.player, self.all_sprites, self.font)
...
class DialogBoxSprite(pygame.sprite.Sprite):
	def __init__(self, message, character, groups, font):
		super().__init__(groups)
		self.z = WORLD_LAYERS['top']
		...
		self.image = surf
		self.rect = self.image.get_frect(midbottom = character.rect.midbottom + vector(0,WINDOW_HEIGHT//2.5))